 ...
 * @version 1.0
 */
public class JsonProcurarInstitucionalSocialPorRecurso extends javax.servlet.http.HttpServlet {
    ...
    protected void processRequest(javax.servlet.http.HttpServletRequest request, javax.servlet.http.HttpServletResponse response)
        ...
        try {
            ...
            if (com.settings.GerenciadorAcesso.checkAuthentication(token, userKey)) {
                ...
                if (institucional.getSocial().getId() > 0) {
                    
                    pgsql.sys.urbano.InstitucionalSocialDAO institucionalDAO = new pgsql.sys.urbano.InstitucionalSocialDAO();
                    java.util.List<com.sys.urbano.InstitucionalSocial> institucionais = institucionalDAO.procurarInstitucionalSocialPorIDRecurso(institucional.getSocial());
                    ...
                    message.setCode(2);
                    message.setMessage("Não foi possível obter os dados institucionais!");
            ...
            com.sys.Message message = new com.sys.Message();
            message.setCode(0);
            message.setMessage("O servidor não pôde obter os dados institucionais!");
            com.data.MessageManager messMgr = new com.data.MessageManager();
            out.print(messMgr.parseJson(message));
            ...
            com.sys.Message message = new com.sys.Message();
            message.setCode(0);
            message.setMessage("O banco de dados retornou um erro durante a seleção dos dados institucionais.<br />Contate o administrador do sistema!");
            com.data.MessageManager messMgr = new com.data.MessageManager();
            out.print(messMgr.parseJson(message));
