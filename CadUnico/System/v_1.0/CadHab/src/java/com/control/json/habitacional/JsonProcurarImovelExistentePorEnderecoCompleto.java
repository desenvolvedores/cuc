 ...
 * @version 1.0
 */
public class JsonProcurarImovelExistentePorEnderecoCompleto extends javax.servlet.http.HttpServlet {
    ...
    protected void processRequest(javax.servlet.http.HttpServletRequest request, javax.servlet.http.HttpServletResponse response)
        ...
        try {
            ...
            if (com.settings.GerenciadorAcesso.checkAuthentication(token, userKey)) {
                ...
                if (! imovel.getEndereco().getTipoLogradouro().isEmpty() && ! imovel.getEndereco().getLogradouro().isEmpty() 
                        && ! imovel.getEndereco().getNumero().isEmpty() && ! imovel.getEndereco().getCEP().isEmpty() 
                        && ! imovel.getEndereco().getBairro().isEmpty() && imovel.getEndereco().getIdImovel() > 0 
                        && imovel.getEndereco().getId() > 0) {
                    
                    pgsql.sys.habitacional.ImovelDAO imovelDAO = new pgsql.sys.habitacional.ImovelDAO();
                    java.util.List<com.sys.habitacional.Imovel> imoveis = imovelDAO.procurarImovelExistentePorEnderecoCompleto(imovel);
                    out.print(jsonMgr.parseJsonListaImovel(imoveis));
                    
                } else {
                    
                    com.sys.Message message = new com.sys.Message();
                    message.setCode(2);
                    message.setMessage("Você precisa informar os dados de endereço do imóvel para realizar a pesquisa!");
            ...
            com.sys.Message message = new com.sys.Message();
            message.setCode(0);
            message.setMessage("O servidor não pôde obter os dados dos imóveis!");
            com.data.MessageManager messMgr = new com.data.MessageManager();
            out.print(messMgr.parseJson(message));
            ...
            com.sys.Message message = new com.sys.Message();
            message.setCode(0);
            message.setMessage("O banco de dados retornou um erro durante a seleção dos dados dos imóveis.<br />Contate o administrador do sistema!");
            com.data.MessageManager messMgr = new com.data.MessageManager();
            out.print(messMgr.parseJson(message));
