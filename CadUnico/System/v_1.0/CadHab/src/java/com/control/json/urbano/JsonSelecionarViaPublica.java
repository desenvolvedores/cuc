 ...
 * @version 1.0
 */
public class JsonSelecionarViaPublica extends javax.servlet.http.HttpServlet {
        ...
        try {
            ...
            if (com.settings.GerenciadorAcesso.checkAuthentication(token, userKey)) {
                ...
                if (via.getId() > 0) {
                    ...
                    via = viaDAO.selecionarViaPublicaPorID(via.getId());
                    out.print(jsonMgr.parseJson(via));
                    
                } else {
                    
                    com.sys.Message message = new com.sys.Message();
                    message.setCode(2);
                    message.setMessage("Não foi possível obter os dados da via pública!");
