 ...
 * @version 1.0
 */
public class JsonAtualizarNucleo extends javax.servlet.http.HttpServlet {
        ...
        try {
            ...
            if (com.settings.GerenciadorAcesso.checkAuthentication(token, userKey)) {
                ...
                if (! nucleo.getNome().isEmpty() && ! nucleo.getOrigem().isEmpty() && ! nucleo.getOcupacao().isEmpty() 
                        ...
                        && ! nucleo.getTransportePublico().isEmpty() && ! nucleo.getAdensamento().isEmpty() && nucleo.getId() > 0) {
            ...
            com.sys.Message message = new com.sys.Message();
            message.setCode(0);
            message.setMessage("O servidor não pôde obter os dados do núcleo para efetuar a atualização!");
            com.data.MessageManager messMgr = new com.data.MessageManager();
            out.print(messMgr.parseJson(message));
            ...
            com.sys.Message message = new com.sys.Message();
            message.setCode(0);
            message.setMessage("O banco de dados retornou um erro durante a atualização dos dados do núcleo.<br />Contate o administrador do sistema!");
            com.data.MessageManager messMgr = new com.data.MessageManager();
            out.print(messMgr.parseJson(message));
