 ...
 * @version 1.0
 */
public class JsonProcurarEmpresaPorRazaoSocial extends javax.servlet.http.HttpServlet {
        ...
        try {
            ...
            if (com.settings.GerenciadorAcesso.checkAuthentication(token, userKey)) {
                ...
                if (! empresa.getRazaoSocial().isEmpty()) {
                    ...
                    java.util.List<com.sys.social.Empresa> empresas = empresaDAO.procurarEmpresaPorRazaoSocial(empresa.getRazaoSocial());
                    out.print(jsonMgr.parseJsonListaEmpresa(empresas));
                    
                } else {
                    
                    com.sys.Message message = new com.sys.Message();
                    message.setCode(2);
                    message.setMessage("Você precisa informar a razão social da empresa para efetuar a pesquisa!");
