 ...
 * @version 1.0
 */
public class JsonSelecionarTelefoneIndividuo extends javax.servlet.http.HttpServlet {
        ...
        try {
            ...
            if (com.settings.GerenciadorAcesso.checkAuthentication(token, userKey)) {
                ...
                if (telefone.getId() > 0) {
                    ...
                    telefone = telefoneDAO.selecionarTelefonePorID(telefone.getId());
                    out.print(jsonMgr.parseJson(telefone));
                    
                } else {
                    
                    com.sys.Message message = new com.sys.Message();
                    message.setCode(2);
                    message.setMessage("Não foi possível obter os dados do telefone!");
            ...
            com.sys.Message message = new com.sys.Message();
            message.setCode(0);
            message.setMessage("O servidor não pôde obter os dados do telefone!");
            com.data.MessageManager messMgr = new com.data.MessageManager();
            out.print(messMgr.parseJson(message));
            ...
            com.sys.Message message = new com.sys.Message();
            message.setCode(0);
            message.setMessage("O banco de dados retornou um erro durante a seleção dos dados do telefone.<br />Contate o administrador do sistema!");
            com.data.MessageManager messMgr = new com.data.MessageManager();
            out.print(messMgr.parseJson(message));
