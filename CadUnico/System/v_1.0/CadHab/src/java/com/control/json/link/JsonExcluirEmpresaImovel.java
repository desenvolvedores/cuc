 ...
 * @version 1.0
 */
public class JsonExcluirEmpresaImovel extends javax.servlet.http.HttpServlet {
    ...
    protected void processRequest(javax.servlet.http.HttpServletRequest request, javax.servlet.http.HttpServletResponse response)
        ...
        try {
            ...
            if (com.settings.GerenciadorAcesso.checkAuthentication(token, userKey)) {
                ...
                if (empresa.getImoveis().size() > 0 && empresa.getId() > 0) {

                    pgsql.sys.link.EmpresaImovelDAO empresaImovelDAO = new pgsql.sys.link.EmpresaImovelDAO();
                    
                    for (com.sys.habitacional.Imovel cursor : empresa.getImoveis()) {

                        if (empresaImovelDAO.excluirEmpresaImovel(empresa, cursor) > 0) {
                    ...
                    message.setCode(2);
                    message.setMessage("Algumas das informações necessárias para a exclusão do imóvel da empresa não possuem valores definidos!");
            ...
            com.sys.Message message = new com.sys.Message();
            message.setCode(0);
            message.setMessage("O servidor não pôde obter os dados do imóvel da empresa para efetuar a exclusão!");
            com.data.MessageManager messMgr = new com.data.MessageManager();
            out.print(messMgr.parseJson(message));
            ...
            com.sys.Message message = new com.sys.Message();
            message.setCode(0);
            message.setMessage("O banco de dados retornou um erro durante a exclusão dos dados do imóvel da empresa.<br />Contate o administrador do sistema!");
            com.data.MessageManager messMgr = new com.data.MessageManager();
            out.print(messMgr.parseJson(message));
