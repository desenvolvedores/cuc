 ...
 * @version 1.0
 */
public class JsonAtualizarEnderecoImovel extends javax.servlet.http.HttpServlet {
        ...
        try {
            ...
            if (com.settings.GerenciadorAcesso.checkAuthentication(token, userKey)) {
                ...
                if (! endereco.getTipoLogradouro().isEmpty() && ! endereco.getLogradouro().isEmpty() && ! endereco.getNumero().isEmpty() 
                        ...
                        && ! endereco.getTipoArea().isEmpty() && endereco.getImovel().getId() > 0 && endereco.getId() > 0) {

                    pgsql.sys.habitacional.EnderecoImovelDAO enderecoDAO = new pgsql.sys.habitacional.EnderecoImovelDAO();

                    if (enderecoDAO.alterarEnderecoImovel(endereco) > 0) {

                        com.db.DBConnection.getInstance().getConnection().commit();
                        com.sys.Message message = new com.sys.Message();
                        message.setCode(1);
                        message.setMessage("Os dados do endereço do imóvel foram atualizados com sucesso!");
                    ...
                    com.sys.Message message = new com.sys.Message();
                    message.setCode(2);
                    message.setMessage("Algumas das informações necessárias para a atualização dos dados do endereço do imóvel não possuem valores definidos!");
                    com.data.MessageManager messMgr = new com.data.MessageManager();
                    out.print(messMgr.parseJson(message));
            ...
            com.sys.Message message = new com.sys.Message();
            message.setCode(0);
            message.setMessage("O servidor não pôde obter os dados do endereço do imóvel para efetuar a atualização!");
            com.data.MessageManager messMgr = new com.data.MessageManager();
            out.print(messMgr.parseJson(message));
            ...
            com.sys.Message message = new com.sys.Message();
            message.setCode(0);
            message.setMessage("O banco de dados retornou um erro durante a atualização dos dados do endereço do imóvel.<br />Contate o administrador do sistema!");
            com.data.MessageManager messMgr = new com.data.MessageManager();
            out.print(messMgr.parseJson(message));
